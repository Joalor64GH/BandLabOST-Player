...
class ModsState extends FlxState
{
	...
	override function update(elapsed:Float)
	{
		...
		{
			if (!Data.disabledMods.contains(ModCore.trackedMods[curSelected].id))
			{
				Data.disabledMods.push(ModCore.trackedMods[curSelected].id);
				FlxG.save.flush();
				changeSelection();
			}
			else
			{
				Data.disabledMods.remove(ModCore.trackedMods[curSelected].id);
				FlxG.save.flush();
				changeSelection();
			}
		}
	...
	private function changeSelection(change:Int = 0)
	{
		...
		for (i in 0...iconArray.length)
		{
			...
			if (!Data.disabledMods.contains(ModCore.trackedMods[i].id))
				iconArray[i].alpha = 1;
		}
